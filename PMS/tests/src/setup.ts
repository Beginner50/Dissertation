...
export default async function globalSetup(config: FullConfig) {
  ...
  const studentAPI = await getAuthorizedAPI("user_main_student@uni.com");
  ...
  const stubDeliverable: DeliverableFile = {
    ...
    file: base64js.fromByteArray(
      new Uint8Array(
        fs.readFileSync(
          path.resolve(__dirname, "../test-assets/deliverables/DataCollectionReport.pdf"),
        ),
      ),
    ),
  };
  ...
  await studentAPI.post(
    `api/users/${studentID}/projects/${projectID}/tasks/${titleTaskMap.get("task_completed")?.taskID}/staged-deliverable`,
    { data: stubDeliverable },
  );
  await studentAPI.post(
    `api/users/${studentID}/projects/${projectID}/tasks/${titleTaskMap.get("task_completed")?.taskID}/staged-deliverable/submit`,
  );
