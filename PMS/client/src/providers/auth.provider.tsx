...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    */
    const authorizedAPI = useMemo(() => ky.extend({
            ...
            beforeError: [
                async (error) => {
                    ...
                    if (error.response?.status === 401) {
                        ...
                        const response = await ky.post(`${window.location.origin}/api/auth/refresh-token`);
                        ...
                        const data = await response.json<Omit<AuthState, "isAuthenticated">>();
    ...
    const signIn = useCallback(async (userData: Record<string, any>): Promise<AuthState> => {
        const response: Omit<AuthState, "isAuthenticated"> =
            await ky.post(`${window.location.origin}/api/auth/signin`, { json: userData }).json();
